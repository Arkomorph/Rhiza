...
export default function EdgePropModal({
  ...
  const title = isEdit ? "Modifier propriété de relation" : "Nouvelle propriété de relation";
  const subtitle = <>Sur la relation <span style={{ fontFamily: "'JetBrains Mono', monospace", color: C.text }}>{targetEdge?.label}</span></>;

  return (
    <ModalShell
      title={title}
      subtitle={subtitle}
      onClose={() => setEdgePropModal(null)}
      width={560}
    >
