...
export default function ExpectationModal({
  ...
  const title = isEdit ? "Modifier attente ontologique" : "Nouvelle attente ontologique";
  const subtitle = <>Sur <span style={{ fontFamily: "'JetBrains Mono', monospace", color: C.text }}>{targetLabel}</span> · pré-remplira les patterns au Step 3 d'une source ciblant ce type</>;

  return (
    <ModalShell
      title={title}
      subtitle={subtitle}
      onClose={() => setExpectationModal(null)}
      width={600}
    >
