...
export default function DerivedPropModal({
  ...
  const title = isEdit ? "Modifier propriété dérivée" : "Nouvelle propriété dérivée";
  const subtitle = <>Sur <span style={{ fontFamily: "'JetBrains Mono', monospace", color: C.text }}>{targetLabel}</span> · D10 propriété d'analyse</>;

  return (
    <ModalShell
      title={title}
      subtitle={subtitle}
      onClose={() => setDerivedPropModal(null)}
      width={560}
    >
      {/* Body — formulaire */}
