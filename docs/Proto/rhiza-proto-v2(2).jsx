...
// ─── Tokens ──────────────────────────────────────────────────────────
const C = {
  bg: "#fafaf8", surface: "#fff", alt: "#f4f3f0",
  border: "#e4e2dc", blight: "#eceae4",
  text: "#1a1a18", muted: "#6b6964", faint: "#9e9b94",
  accent: "#2d5a27", accentL: "#e8f0e7",
  info: "#2b5a8a", infoL: "#edf4fb",
  warn: "#8a6914", warnL: "#fdf6e3",
  error: "#9a2c2c",
  edit: "#d000ff", editL: "#fbf0ff",     // magenta-violet ultra saturé — actions d'édition (parcours 5 §I6+)
};
const TC = {
  Suisse: "#406d6c",    // vert-gris foncé (froid)
  Canton: "#c5604d",    // orange-cuivré (chaud)
  Commune: "#5696a4",   // bleu acier (froid)
  Quartier: "#c49c4e",  // ambre (chaud)
  Parcelle: "#6f8270",  // vert olive (froid)
  Bâtiment: "#846056",  // brun (chaud)
  Logement: "#807184",  // violet-gris (froid)
  Pièce: "#ce5561",     // rouge-rose (chaud)
};

// ─── Palette Acteurs ─────────────────────────────────────────────────
// Source : palette fournie par Jo (Avril 2026). 12 teintes destinées aux
// sous-types d'Acteur. Archivée dans Notion → page « Palettes du projet ».
// Stockée comme liste brute pour l'instant ; assignation aux sous-types
// (Individu, Menage, Indivision, Droit_prive, Droit_public, Collectif_informel,
// Individu_vivant, Population, Ecosysteme, etc.) à trancher quand le besoin
// se présentera (visualisation, décoration sidebar Schéma, etc.).
const AC_PALETTE = [
  { hex: "#f0815c", name: "saumon",          rgb: [240, 129,  92] },
  { hex: "#773673", name: "aubergine",       rgb: [119,  54, 116] },
  { hex: "#e8474d", name: "rouge_corail",    rgb: [233,  71,  78] },
  { hex: "#0588b0", name: "bleu_cyan",       rgb: [ 23, 137, 177] },
  { hex: "#14966b", name: "vert_emeraude",   rgb: [ 31, 151, 108] },
  { hex: "#b1ae37", name: "olive",           rgb: [177, 175,  55] },
  { hex: "#f9b74d", name: "ambre",           rgb: [250, 183,  77] },
  { hex: "#424e59", name: "ardoise",         rgb: [ 66,  78,  89] },
  { hex: "#846056", name: "brun",            rgb: [132,  97,  86] },
  { hex: "#838383", name: "gris_neutre",     rgb: [131, 131, 131] },
  { hex: "#4d779e", name: "bleu_acier",      rgb: [ 69, 183, 158] },
  { hex: "#5ec4e4", name: "bleu_pale",       rgb: [ 93, 196, 229] },
];
const F = { title: "'Geist', 'DM Sans', sans-serif", body: "'Roboto Condensed', sans-serif", logo: "'DM Sans', sans-serif" };
...
// Paliers d'atténuation — ordre de rendu : cascade (le plus clair) → active (pleine couleur)
// Résout une couleur pour n'importe quel chemin de l'ontologie.
// Stratégie : si le chemin est dans Territoire, on lit TC[type feuille] ou TC[parent intermédiaire] ;
// si Acteur, on prend AC_PALETTE par hash du chemin (assignation déterministe stable) ;
// Flux et Décision ont leur propre couleur fixe.
function colorForOntologyPath(path) {
  if (!path || path.length === 0) return "#9e9b94";
  const root = path[0];
  if (root === "Territoire") {
    // Cherche d'abord la couleur la plus spécifique (la dernière qui matche TC)
  ...
  if (root === "Acteur") {
    // Hash déterministe sur le chemin pour piocher dans AC_PALETTE
    const key = path.join(":");
    let h = 0;
...
// Bibliothèque Phosphor (https://phosphoricons.com/) — style "duotone".
// Chaque icône a deux paths : light (opacity 0.2 = teinte de fond) + dark (contour).
// Utilisation : <Icon name="pencil" size={14} color={C.edit} />
const PHOSPHOR_ICONS = {
  pencil: {
    light: "M227.31,73.37,182.63,28.69a16,16,0,0,0-22.63,0L36.69,152A15.86,15.86,0,0,0,32,163.32V208a16,16,0,0,0,16,16H92.69A15.86,15.86,0,0,0,104,219.31L227.31,96A16,16,0,0,0,227.31,73.37Z",
    dark: "M227.31,73.37,182.63,28.68a16,16,0,0,0-22.63,0L36.69,152A15.86,15.86,0,0,0,32,163.31V208a16,16,0,0,0,16,16H92.69A15.86,15.86,0,0,0,104,219.31L227.31,96a16,16,0,0,0,0-22.63ZM51.31,160,136,75.31,152.69,92,68,176.68ZM48,179.31,76.69,208H48Zm48,25.38L79.31,188,164,103.31,180.69,120Zm96-96L147.31,64l24-24L216,84.68Z",
  },
  trash: {
    light: "M200,56V208a8,8,0,0,1-8,8H64a8,8,0,0,1-8-8V56Z",
    dark: "M216,48H176V40a24,24,0,0,0-24-24H104A24,24,0,0,0,80,40v8H40a8,8,0,0,1,0-16h176a8,8,0,0,1,0,16ZM96,40a8,8,0,0,1,8-8h48a8,8,0,0,1,8,8v8H96ZM208,208a16,16,0,0,1-16,16H64a16,16,0,0,1-16-16V64H208ZM112,104v64a8,8,0,0,1-16,0V104a8,8,0,0,1,16,0Zm48,0v64a8,8,0,0,1-16,0V104a8,8,0,0,1,16,0Z",
  },
  plusCircle: {
    light: "M224,128a96,96,0,1,1-96-96A96,96,0,0,1,224,128Z",
    dark: "M128,24A104,104,0,1,0,232,128,104.13,104.13,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm40-88a8,8,0,0,1-8,8H136v24a8,8,0,0,1-16,0V136H96a8,8,0,0,1,0-16h24V96a8,8,0,0,1,16,0v24h24A8,8,0,0,1,168,128Z",
  },
  check: {
    light: "M232,128A104,104,0,1,1,128,24,104.11,104.11,0,0,1,232,128Z",
    dark: "M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm45.66-114.34a8,8,0,0,1,0,11.32l-56,56a8,8,0,0,1-11.32,0l-24-24a8,8,0,0,1,11.32-11.32L112,148.69l50.34-50.35A8,8,0,0,1,173.66,101.66Z",
  },
  x: {
    light: "M224,128a96,96,0,1,1-96-96A96,96,0,0,1,224,128Z",
    dark: "M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm37.66-125.66L139.31,128l26.35,26.34a8,8,0,0,1-11.32,11.32L128,139.31l-26.34,26.35a8,8,0,0,1-11.32-11.32L116.69,128,90.34,101.66a8,8,0,0,1,11.32-11.32L128,116.69l26.34-26.35a8,8,0,0,1,11.32,11.32Z",
  },
};
function Icon({ name, size = 14, color = "currentColor", style = {} }) {
  const paths = PHOSPHOR_ICONS[name];
  if (!paths) return null;
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill={color} viewBox="0 0 256 256" style={{ display: "inline-block", verticalAlign: "middle", ...style }}>
      <path d={paths.light} opacity="0.2" />
      <path d={paths.dark} />
    </svg>
  );
}

// Sous-types Territoire — suit les cellules D3.2 (Radu et al.)
// Liste ouverte : en production, vient dynamiquement du Schéma
const TYPES = ["Canton", "Commune", "Quartier", "Parcelle", "Bâtiment", "Logement", "Pièce"];
// Strict imbrication — Bâtiment always in Parcelle, single canonical chain
const CHILDREN_OF = {
  "Suisse": ["Canton"],
  "Canton": ["Commune"],
  "Commune": ["Quartier"],
...
const INDENT = 20;
// Alignement de la pastille de cascade avec la pastille d'une carte enfant direct
// = INDENT + trait(16) + gap(8) + demi-pastille-carte(5) − demi-pastille-cascade(4) = 45
const CASCADE_OFFSET = 45;
// Note : Socio-bio-tope (Cell E) et Socio-eco-tope (Cell F) sont des
// niveaux analytiques qui émergent des données — ajoutables via Schéma.
// Hub social = propriété booléenne d'un nœud Territoire, pas un sous-type.
...
const ROOT = { id: "suisse", nom: "Suisse", type: "Suisse", status: "active", permanent: true };
...
const ONTOLOGY_TYPES = ONTOLOGY_GROUPS.flatMap(g => g.types);

// ─── Ontologie des arêtes (D1–D9, Annexe 02) ─────────────────────────
// Chaque arête : direction canonique (from → to), où "from" et "to" sont des familles de types.
// Les familles : "Territoire" (tous les sous-types), "Acteur", "Flux", "Décision".
// Un pattern pourra inverser la direction si l'utilisateur le veut, indépendamment de la forme canonique.
...
// Familles de types pour filtrer les arêtes
const TYPE_FAMILY = (t) => {
  if (["Canton", "Commune", "Quartier", "Parcelle", "Bâtiment", "Logement", "Pièce", "Suisse"].includes(t)) return "Territoire";
  return t; // Acteur, Flux, Décision
};
// Arêtes compatibles étant donné deux familles (dans un sens ou l'autre)
// Alias pour rétrocompatibilité du code patterns/Step 3 qui référence EDGE_TYPES.
// Le parcours 5 Schéma utilise un state mutable séparé (edgeTypes) pour permettre l'édition.
const EDGE_TYPES = INITIAL_EDGE_TYPES;
const compatibleEdges = (famA, famB) =>
  EDGE_TYPES.filter(e =>
...
const compatibleSpatialOps = (sourceKind, targetKind) => {
  if (!sourceKind || !targetKind) return [];
  return SPATIAL_OPS[sourceKind]?.[targetKind] || [];
};

// ─── Hiérarchie complète des types et sous-types (Annexe 02 D8) ──────
// Source de vérité pour le parcours 5 Schéma. Aplati en ONTOLOGY_TYPES pour rétrocompat.
// Chaque nœud porte : key, label, children?, props? (héritées par les descendants).
// Format prop : { key, label, type, enum_values?, enum_source?, natural_key?, geomKind?, notes? }
// Source de vérité initiale — sera copiée en state mutable au démarrage du composant App.
const INITIAL_ONTOLOGY_TREE = {
  Territoire: {
...
const flattenOntology = (tree, path = [], result = {}) => {
...
// Cherche récursivement le chemin d'un type dans l'arbre ontologique.
// Ex. findPathForType(tree, "Bâtiment") → ["Territoire", "Bâtiment"]
// Ex. findPathForType(tree, "Personne_morale") → ["Acteur", "Humain", "Groupe", "Personne_morale"]
// Retourne [type] si non trouvé (fallback prudent).
const findPathForType = (tree, type, prefix = []) => {
  for (const [key, node] of Object.entries(tree || {})) {
...
// Calcule les propriétés effectives d'un nœud en remontant l'héritage dans un arbre donné.
// Le tree est passé en paramètre (state mutable depuis App).
const getEffectiveProps = (tree, path) => {
  const result = [];
...
// ─── Mutations sur l'arbre ontologique (immutables, pour state) ──────
// Applique un mutateur sur le nœud à path et retourne un nouvel arbre.
const updateNodeAtPath = (tree, path, mutator) => {
...
// Helpers métier — ajout, modification, suppression de propriété intrinsèque
const treeAddProp    = (tree, path, prop)         => updateNodeAtPath(tree, path, n => ({ ...n, props: [...(n.props || []), prop] }));
const treeUpdateProp = (tree, path, key, newProp) => updateNodeAtPath(tree, path, n => ({ ...n, props: (n.props || []).map(p => p.key === key ? newProp : p) }));
const treeRemoveProp = (tree, path, key)          => updateNodeAtPath(tree, path, n => ({ ...n, props: (n.props || []).filter(p => p.key !== key) }));

// Helpers métier — édition de la hiérarchie elle-même (créer / renommer / supprimer un sous-type)
const treeAddSubtype = (tree, parentPath, subtype) => updateNodeAtPath(tree, parentPath, n => ({
  ...n,
  children: { ...(n.children || {}), [subtype.key]: subtype },
}));const treeUpdateSubtype = (tree, path, partial) => updateNodeAtPath(tree, path, n => ({ ...n, ...partial }));
// Renommage de la clé : on retire l'ancienne entrée et on en ajoute une nouvelle dans le parent.
// path = chemin du nœud à renommer ; newKey = nouvelle clé.
const treeRenameSubtype = (tree, path, newKey, partial) => {
  if (path.length === 0) return tree;
  const parentPath = path.slice(0, -1);
  const oldKey = path[path.length - 1];
  if (newKey === oldKey) {
    // Pas de renommage de clé, juste mise à jour des autres champs
...
// Suppression : retire le sous-type (et tous ses descendants) du parent.
const treeRemoveSubtype = (tree, path) => {
  if (path.length === 0) return tree;
  const parentPath = path.slice(0, -1);
...
// Compte les descendants (sous-types) sous un nœud, récursivement
...
// ─── Normalisation des valeurs d'enum (parcours 5 §I8) ───────────────
// Le modèle interne des valeurs d'enum est un objet { value, label, code_externe }.
// Mais pour la lisibilité du code source, on accepte aussi des strings simples.
// Cette fonction normalise vers le format objet à l'usage.
//
// Format canonique : { value: "8011", label: "Avant 1919", code_externe: { regbl: "8011" } }
// Format compact accepté : "8011" (label = value, code_externe vide)
const normEnumValue = (v) => {
...
const normEnumValues = (arr) => (arr || []).map(normEnumValue);
// Sérialise une liste de valeurs en string compact pour l'aperçu (table de propriétés)
const previewEnumValues = (arr, max = 6) => {
...
// ─── Attentes ontologiques (parcours 5 §I7, § WW Annexe 13) ──────────
// Une attente est une déclaration au niveau du Schéma : ce nœud est attendu relié
// par telle arête à tel autre type, dans tel sens, avec telle obligation.
// Les attentes héritent comme les propriétés intrinsèques. Une attente du parent
// peut être surchargée par un descendant en réutilisant la même signature :
// (edgeKey + direction + otherSidePath joint).
//
// Format : { edgeKey, direction: "outgoing"|"incoming", otherSide: ["Type", "Sous-type"],
//            obligation: "hard"|"soft", multiplicity: "one"|"many",
//            defaultMode: "linkOrCreateField"|"linkOrCreateGeneric",
//            notes: "..." }
// Note : la clé de matching n'est pas portée par l'attente — elle dépend des propriétés
// de la source d'import (donc inconnue au moment du Schéma) et sera dérivée au Step 3
// depuis les propriétés `natural_key` du type cible.

// Signature canonique d'une attente — utilisée pour détecter les surcharges
const expectationSignature = (exp) =>
  `${exp.edgeKey}|${exp.direction}|${(exp.otherSide || []).join(":")}`;

// ─── Helpers de validation des patterns (parcours 2 Step 3) ───────────
// Définis au niveau global pour être utilisés à la fois dans le rendu des patterns
// et dans le footer de la modale (calcul des manques pour activer le bouton Suivant).

// Un pattern est complet si : type + arête + (mappings + dédup) ou (au moins une propriété)
const isPatternCompleteHelper = (p) => {
...
// Premier élément manquant pour qu'un pattern soit complet, dans l'ordre de l'interface.
const firstMissingHintHelper = (p) => {
...
// Liste des éléments manquants pour qu'un step donné de la modale source soit validable.
// Renvoie un array vide si le step est complet (donc validable).
const getStepMissing = (step, draft) => {
  if (!draft) return [];
...
// Calcule les attentes effectives d'un nœud en remontant l'héritage.
// Comme getEffectiveProps, l'enfant le plus spécifique gagne en cas de surcharge.
const getEffectiveExpectations = (tree, path) => {
  const result = [];
  let currentLevel = tree;
  for (let i = 0; i < path.length; i++) {
    const key = path[i];
    ...
    if (node.expectedEdges) {
      for (const exp of node.expectedEdges) {
        ...
        if (idx >= 0) {
          // Surcharge — on remplace mais on garde une trace que l'attente est aussi présente plus haut
...
// Mutateurs sur le champ expectedEdges d'un nœud (parallèles aux helpers props)
const treeAddExpectation    = (tree, path, exp)        => updateNodeAtPath(tree, path, n => ({ ...n, expectedEdges: [...(n.expectedEdges || []), exp] }));
const treeUpdateExpectation = (tree, path, sig, newExp) => updateNodeAtPath(tree, path, n => ({ ...n, expectedEdges: (n.expectedEdges || []).map(e => expectationSignature(e) === sig ? newExp : e) }));
const treeRemoveExpectation = (tree, path, sig)        => updateNodeAtPath(tree, path, n => ({ ...n, expectedEdges: (n.expectedEdges || []).filter(e => expectationSignature(e) !== sig) }));

// Pour un chemin donné, retourne les propriétés dérivées applicables avec leur distance d'héritage.
// Une dérivée définie sur Acteur s'applique à Acteur, Acteur:Humain, etc. — héritage descendant.
const getEffectiveDerivedProps = (allDerived, path) => {
  return allDerived
...
export default function App() {
  ...
  // Propriétés dérivées (D10) — éditables car ce sont des "propriétés d'analyse" propres au projet.
  // Pré-remplies avec les 5 cas validés couvrant les 3 mécanismes.
  ...
  if (page === "login") return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: F.body, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "40px 48px", width: 340, textAlign: "center" }}>
        ...
        <button onClick={() => setPage("app")} style={{ width: "100%", padding: "10px", fontSize: 13, fontWeight: 600, border: "none", borderRadius: 7, background: C.accent, color: "#fff", cursor: "pointer", fontFamily: F.body }}>Se connecter</button>
      </div>
    </div>
  );
  ...
  // ─── Canonical chain for auto-creating intermediaries ──────────────
  // Strict imbrication : Bâtiment toujours dans Parcelle, Parcelle toujours dans Quartier, etc.
  const CANONICAL = ["Suisse", "Canton", "Commune", "Quartier", "Parcelle", "Bâtiment", "Logement", "Pièce"];

  function getIntermediaryTypes(parentType, targetType) {
  ...
  const handleSubmit = () => {
    ...
    // Mode edit : UPDATE nom (pas d'enchaînement, onglet Configurer accessible à la main)
    ...
    // Mode create : auto-intermédiaires + nœud principal
    ...
    setCreateName("");
    // Nœud créé → on bascule sur l'onglet Configurer dans la même modale
  ...
  const sourceNode = createModal?.nodeId ? nodes.find(n => n.id === createModal.nodeId) : null;
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: F.body }}>
                    ...
                    <div>
                      ...
                      <div style={{ padding: "16px 12px 18px", background: C.alt, borderRadius: 7, marginBottom: 12 }}>
                        <svg viewBox="0 0 400 84" style={{ width: "100%", height: 84, display: "block" }}>
                          ...
                          <text x={48} y={74} textAnchor="middle" fontSize={9} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.06em" fill={C.faint} style={{ textTransform: "uppercase" }}>
                            CE NŒUD
                          </text>

                          {/* Flèche directionnelle interactive — outgoing : → ; incoming : ← */}
                          <text x={200} y={18} textAnchor="middle" fontSize={11} fontStyle="italic" fontFamily="'Roboto Condensed', sans-serif" fill={C.muted}>
                          ...
                          <text x={352} y={74} textAnchor="middle" fontSize={9} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.06em" fill={C.faint} style={{ textTransform: "uppercase" }}>
                            CIBLE
                          </text>
                        </svg>
                      </div>
      ...
      {edgePropModal && (() => {
        ...
        // Vérifier l'unicité de la clé sur l'arête (parmi les propriétés universelles + spécifiques)
      ...
      {subtypeModal && (() => {
        ...
        // Validation : key PascalCase ou snake_case (CamelCase autorisé pour cohérence avec sous-types existants), label non vide
        const keyValid = /^[A-Za-z][A-Za-z0-9_]*$/.test(draft.key || "");
        const labelValid = (draft.label || "").trim().length > 0;

        // Vérifier l'unicité de la clé chez le parent
        ...
        const handleSave = () => {
          if (!formValid || keyConflict) return;
          if (isEdit) {
            // Renommage / redéfinition. Si la clé change, le chemin courant change aussi.
            const newPath = [...path.slice(0, -1), draft.key];
            setOntologyTree(treeRenameSubtype(ontologyTree, path, draft.key, {
              label: draft.label,
              description: draft.description,
            }));
            // Mettre à jour la sélection si on modifiait le nœud sélectionné ou un de ses ancêtres
            ...
            // Création d'un nouveau sous-type sous parentPath
            const subtype = {
              key: draft.key,
              label: draft.label,
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
            <div style={{ width: 520, maxHeight: "85vh", background: C.surface, borderRadius: 14, padding: "28px 32px", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column" }}>
              {/* Header */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexShrink: 0 }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 600, fontFamily: F.title, textTransform: "uppercase" }}>
                    {isEdit ? "Renommer / redéfinir un sous-type" : "Nouveau sous-type"}
                  </div>
                  <div style={{ fontSize: 11, color: C.faint, marginTop: 2 }}>
                    {isEdit
                      ? <span>Sous le parent <span style={{ fontFamily: "'JetBrains Mono', monospace", color: C.text }}>{parentLabel}</span> · le renommage de la clé entraînera une migration des nœuds existants en production.</span>
                      : <span>Sous le parent <span style={{ fontFamily: "'JetBrains Mono', monospace", color: C.text }}>{parentLabel}</span></span>}
                  </div>
                </div>
                <span onClick={() => setSubtypeModal(null)} style={{ cursor: "pointer", display: "inline-flex", padding: 2 }}>
                  <Icon name="x" size={16} color={C.muted} />
                </span>
              </div>
      ...
      {derivedPropModal && (() => {
        ...
        // Validation : key non vide, syntaxe snake_case, label non vide
        const keyValid = /^[a-z][a-z0-9_]*$/.test(draft.key);
        const labelValid = draft.label.trim().length > 0;
        const formValid = keyValid && labelValid;
        ...
        const handleSave = () => {
          ...
          if (isEdit) {
            ...
            setDerivedProps([...derivedProps, {
              id: newId,
              targetPath: [...schemaSelection.path],   // attaché au nœud courant
              ...draft,
            }]);
          }
      ...
      {createModal && (() => {
        ...
        const canConfigure = !!currentNode && !currentNode.placeholder; // onglet Configurer accessible uniquement après nommage
        ...
        const subtitle = createModal.tab === "configure"
          ...
          : (createModal.mode === "edit"
              ? `${createModal.type} · dans ${getParentName(createModal.parentId)}`
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
            <div style={{ width: 480, maxHeight: "85vh", background: C.surface, borderRadius: 14, padding: "28px 32px", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexShrink: 0 }}>
                ...
                <span onClick={() => { setCreateModal(null); setSourceFilter(""); setCreateName(""); }} style={{ fontSize: 14, cursor: "pointer", color: C.muted }}>✕</span>
              </div>
                ...
                <div style={{ flexShrink: 0 }}>
                  ...
                  <div style={{ marginBottom: 14, display: "flex", gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ padding: "8px 14px", fontSize: 13, background: C.alt, borderRadius: 7, color: C.muted, fontWeight: 500 }}>{getParentName(createModal.parentId)}</div>
                    </div>
                  </div>
              ...
              {createModal.tab === "configure" && currentNode && (
                <>
                  ...
                  <div style={{ flex: 1, overflowY: "auto", marginBottom: 12, minHeight: 0 }}>
                    {filteredCat.map(s => {
                      ...
                      return (
                        <div
                          key={s.id}
                          onClick={() => toggleSource(createModal.nodeId, s.id)}
                          style={{
                  ...
                  <div
                    onClick={() => setAddSourceModal({ nodeId: createModal.nodeId })}
      ...
      {archiveModal && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
            <div style={{ width: 440, maxHeight: "85vh", background: C.surface, borderRadius: 14, padding: "28px 32px", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexShrink: 0 }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 600, fontFamily: F.title, textTransform: "uppercase" }}>Archiver {target.placeholder ? `ce ${target.type}` : target.nom}</div>
                  <div style={{ fontSize: 11, color: C.faint, marginTop: 2 }}>{total} objet{total > 1 ? "s" : ""} · archivé{total > 1 ? "s" : ""}, pas détruit{total > 1 ? "s" : ""}</div>
                </div>
                <span onClick={() => setArchiveModal(null)} style={{ fontSize: 14, cursor: "pointer", color: C.muted }}>✕</span>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, flexShrink: 0 }}>
                <button onClick={() => setArchiveModal(null)} style={{ fontSize: 12, padding: "8px 16px", border: `1px solid ${C.border}`, borderRadius: 7, background: C.surface, color: C.muted, cursor: "pointer", fontFamily: F.body }}>Annuler</button>
                <button
                                  ...
                                  <div style={{ padding: "14px 16px" }}>
                                    ...
                                    <div style={{ padding: "16px 12px 18px", background: C.alt, borderRadius: 7, marginBottom: 12 }}>
                                      ...
                                      <svg viewBox="0 0 400 72" style={{ width: "100%", height: 72, display: "block" }}>
                                        ...
                                        <text x={64} y={64} textAnchor="middle" fontSize={8} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.08em" fill={C.faint} style={{ textTransform: "uppercase" }}>
                                          NŒUD IMPORTÉ
                                        </text>
                                        ...
                                        <text x={336} y={64} textAnchor="middle" fontSize={8} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.08em" fill={C.faint} style={{ textTransform: "uppercase" }}>
                                      ...
                                      <>
                                        ...
                                        <div style={{ padding: "14px 16px", background: C.alt, borderRadius: 7, marginBottom: 14 }}>
                                          <svg viewBox="0 0 400 120" style={{ width: "100%", height: 120, display: "block" }}>
                                            ...
                                            <text x={78} y={108} textAnchor="middle" fontSize={8} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.08em" fill={C.faint} style={{ textTransform: "uppercase" }}>
                                              LIGNES DE LA SOURCE
                                            </text>
                                            ...
                                            <text x={310} y={108} textAnchor="middle" fontSize={8} fontFamily="'Roboto Condensed', sans-serif" letterSpacing="0.08em" fill={C.faint} style={{ textTransform: "uppercase" }}>
